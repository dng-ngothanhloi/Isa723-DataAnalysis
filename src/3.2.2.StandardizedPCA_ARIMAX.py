...
#df = pd.read_csv('VNINDEX_standardized.csv', index_col='Date', parse_dates=True)
...
print(f"Ma trận Hiệp phương sai và Ma trận Tương quan có gần giống nhau không (sử dụng np.allclose)? {are_matrices_close}")
...
if not X_Variant.empty:
    ...
    print("Ý nghĩa: Eigenvalues thể hiện phương sai mà mỗi thành phần chính giải thích. Giá trị càng lớn, PC càng quan trọng.")
    ...
    print("Ý nghĩa: Tỷ lệ phương sai tích lũy cho biết tổng phương sai được giải thích bởi các PC. Thường cần >= 80% để đảm bảo dữ liệu giảm chiều tốt.")
    ...
    if cumulative_variance[-1] >= variance_threshold:
        ...
        print("\nNhận xét: PCA cho phép giảm đáng kể số chiều dữ liệu trong khi vẫn giữ lại phần lớn thông tin (phương sai). Các thành phần chính này sẽ là các đặc trưng mới cho các mô hình tiếp theo.")
         ...
         print("          hoặc phương sai phân bổ đều trên nhiều chiều. Việc giảm chiều bằng PCA có thể không hiệu quả nhiều trong trường hợp này nếu mục tiêu là giữ lại tỷ lệ phương sai rất cao.")
...
if 'VNINDEX' in df_cleaned.columns and df_cleaned.shape[1] > 1: # Need VNINDEX and at least one feature for PCA
    ...
    if not X_cleaned.empty and X_cleaned.shape[1] > 0:
        ...
        if n_components_final_pca > 0:
            ...
            loadings_final_df = pd.DataFrame(loadings_final, columns=[f'PC{i+1}' for i in range(n_components_final_pca)], index=X_cleaned.columns)
                    
            # In bảng kết quả PC1~PC5 với top 5 biến và p-value ADF
            table_data = []
            for i in range(5):
                # Tính top 5 biến đóng góp
                abs_loadings = np.abs(loadings_final_df[f'PC{i+1}'])
                total_abs_loadings = abs_loadings.sum()
                top_5_vars = abs_loadings.sort_values(ascending=False).head(5).index
                top_5_contrib = [f"{var} ({(abs_loadings[var] / total_abs_loadings * 100):.2f}%)" for var in top_5_vars]
                top_5_str = ", ".join(top_5_contrib)
            ...
            print("\nBảng thông tin các thành phần chính (PC1~PC5):")
            print(tabulate(table_data, headers=["Thành phần chính", "Phương sai giải thích (%)", "Phương sai tích lũy (%)", "Top 5 biến đóng góp (% đóng góp)", "p-value ADF"], tablefmt="grid"))
...
# 8. Kiểm tra overfitting của PCA
X_train_cleaned, X_test_cleaned = train_test_split(X_cleaned, test_size=0.2, random_state=42)
pca_train_final = PCA(n_components=5)
X_train_pca_final = pca_train_final.fit_transform(X_train_cleaned)
X_test_pca_final = pca_final.transform(X_test_cleaned)
